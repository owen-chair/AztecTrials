Shader "VRChat/Mobile/Standard Lite Custom Door Spill"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        #pragma surface surf StandardVRCNoRealtimeDoorSpill vertex:vert exclude_path:prepass exclude_path:deferred noforwardadd novertexlights noshadow nodynlightmap nolppv noshadowmask
        #pragma skip_variants LIGHTMAP_SHADOW_MIXING

        float4 _UdonTempleDoorPlaneOrigin;
        float4 _UdonTempleDoorPlaneNormal;

        inline half4 LightingStandardVRCNoRealtimeDoorSpill(
            ...
            UnityGI lighting)
        {
            ...
            half3 bakedDiffuse = max(lighting.indirect.diffuse, 0.0h)
                * surface.MinimumBrightness;
            half4 color = half4(bakedDiffuse * surface.Albedo, outputAlpha);
        ...
        inline void LightingStandardVRCNoRealtimeDoorSpill_GI(
            SurfaceOutputStandardVRC surface,
            UnityGIInput data,
        ...
        void surf(Input input, inout SurfaceOutputStandardVRC output)
        {
            ...
            float signedDistance = dot(
                input.worldPos - _UdonTempleDoorPlaneOrigin.xyz,
                _UdonTempleDoorPlaneNormal.xyz
            );
            output.MinimumBrightness = smoothstep(
                -1.0h,
                1.0h,
                -signedDistance * _UdonTempleDoorPlaneNormal.w
            );
        }
        ENDCG
    }
