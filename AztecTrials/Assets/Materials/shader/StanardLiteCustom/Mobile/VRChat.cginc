...
#if defined(_MONOSH)
inline half3 BakeryMonoSHDiffuse(float2 lmUV, half3 normalWorld)
{
...
{
    half oneMinusReflectivity = OneMinusReflectivityFromMetallic(s.Metallic);
    s.Albedo *= oneMinusReflectivity;

    half outputAlpha;
    s.Albedo = PreMultiplyAlpha(s.Albedo, s.Alpha, oneMinusReflectivity, /*out*/ outputAlpha);

    half4 c = half4(gi.indirect.diffuse * s.Albedo, outputAlpha);
...
inline UnityGI UnityGI_BaseVRCNoRealtime(UnityGIInput data, half occlusion, half3 normalWorld)
{
    UnityGI o_gi;
    o_gi.light = data.light;
    o_gi.light.color = 0;
    ...
    #if defined(LIGHTMAP_ON)
        #if defined(_MONOSH)
            o_gi.indirect.diffuse = BakeryMonoSHDiffuse(data.lightmapUV.xy, normalWorld);
        #else
            half3 bakedColor = half3(1.0, 1.0, 1.0);
            half4 bakedColorTex = UNITY_SAMPLE_TEX2D(unity_Lightmap, data.lightmapUV.xy);
            #if defined(FORCE_UNITY_DLDR_LIGHTMAP_ENCODING)
...
{
    gi = UnityGI_BaseVRCNoRealtime(data, s.Occlusion, s.Normal);

    // Minimal NaN/Inf guard.
