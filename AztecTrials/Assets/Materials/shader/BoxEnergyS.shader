Shader "Custom/BoxEnergyS"
{
    ...
    SubShader
    {
        ...
        struct Input
        {
            float2 uv_MainTex;
            float3 worldPos;
            float3 worldNormal; // <- added: needed for rim based on frag normal
        };
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            // Rim mask (replaces UV-border mask):
            //  - 0 when facing camera
            //  - 1 near silhouette (normal perpendicular to view)
            float3 N = normalize(IN.worldNormal);
            float3 V = normalize(UnityWorldSpaceViewDir(IN.worldPos));
            float ndv = saturate(dot(N, V));
            float rim = 1.0 - ndv;

            // Use existing _GlowPower to control sharpness
            float edgeBase = pow(saturate(rim), _GlowPower);
            ...
            // If you want _NoiseAmount to actually scale fuzz strength, apply it here
            // (keeps rim shape but reduces/boosts the noisy breakup)
            edge = lerp(edgeBase, edge, saturate(_NoiseAmount));
